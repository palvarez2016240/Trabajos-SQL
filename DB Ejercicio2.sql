...
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
delimiter $$
create function fn_Raiz(num int) #drop function fn_raiz
	returns int
    reads sql data deterministic
		begin
			declare lit int default 0;
            declare lsup int;
            declare result int default 0;
				repeat 
                set result = 0;
...
select fn_Raiz(64) as Raiz;

#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
delimiter $$
	create function fn_fibonacci(num int) #drop function fn_fibonacci
    returns varchar (256)
    reads sql data deterministic 
    Begin
		declare valor1 int;
                ...
                elseif(num = 2) then 
					set result = '0, 1';
                        ...
                        while (cont < num) do
                            ...
                            set result = concat(result , ' , ', valor3);
                            set valor1 = valor2;
                            set valor2 = valor3;
						end while;
					end if;
...
select fn_fibonacci(10) as Fibonacci;